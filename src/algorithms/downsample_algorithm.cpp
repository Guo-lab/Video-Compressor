...
void DownsampleAlgorithm::reset() {
    m_stats.frames_compressed = 0;
    m_stats.frames_decompressed = 0;
    m_stats.average_compression_ratio = 0.0;
    m_stats.total_compression_time_ms = 0.0;
...
DownsampleAlgorithm::~DownsampleAlgorithm() = default;
 ...
 */
bool DownsampleAlgorithm::initialize(const CompressionConfig &config) {
    m_config = config;
...
/**
 * @brief Compress a video frame. Downsample the image by a factor of 2 or 4
 * @param frame The input video frame to compress
 * @return std::vector<uint8_t> The compressed data
 */
std::vector<uint8_t> DownsampleAlgorithm::compressFrame(const Frame &frame) {
 ...
 * original.
 */
Frame DownsampleAlgorithm::decompressFrame(const std::vector<uint8_t> &compressed_data) {
    auto start_time = std::chrono::high_resolution_clock::now();
    ...
    int downsampled_w = original_width / m_downsample_factor;
    int downsampled_h = original_height / m_downsample_factor;
    cv::Mat downsampled_mat(downsampled_h, downsampled_w, CV_8UC3);
    std::memcpy(downsampled_mat.data, compressed_data.data() + METADATA_BYTES,
                compressed_data.size() - METADATA_BYTES);
 ...
 */
std::string DownsampleAlgorithm::getStats() const {
    std::stringstream ss;
    ss << "Downsample Algorithm Statistics:" << std::endl
       << "  Downsample factor: " << m_downsample_factor << std::endl
 ...
 * to the OpenCV Mat.
 */
cv::Mat DownsampleAlgorithm::frameToMat(const Frame &frame) {
 ...
 */
void DownsampleAlgorithm::updateCompressionStats(const cv::Mat &original, const cv::Mat &compressed) {
 ...
 */
void DownsampleAlgorithm::copyMatToBuffer(const cv::Mat &mat, int w, int h, uint8_t *buffer) {
 ...
 */
Frame DownsampleAlgorithm::matToFrame(const cv::Mat &mat) {
    Frame frame;
